...
describe('authorName', () => {
  it('takes the display name out of "Name <email>"', () => {
    expect(authorName('Francesco Lodolo <flod@lodolo.net>')).toBe('Francesco Lodolo');
  });
...
describe('commitsInRange', () => {
  ...
  it('drops the base push', () => {
    const range = commitsInRange([push(3, 'ccc'), push(2, 'bbb'), push(1, 'aaa')], 'aaa', false);
  ...
  it('keeps everything when the base push is not in the response', () => {
    const range = commitsInRange([push(3, 'ccc'), push(2, 'bbb')], 'zzz', false);
  ...
  it('flattens a multi-commit push newest first, preserving order', () => {
    const range = commitsInRange([push(2, 'tip', { named: ['tip', 'mid', 'old'] })], 'x', false);
      ...
      revisions: [
        {
          ...
          comments: 'Bug 2058227 - Integrate the thing r=reviewer\n\nDepends on bug 999999',
        },
      ],
    };
...
describe('commitTitle', () => {
  ...
  it('strips the leading bug when it is the one being linked', () => {
    expect(commitTitle(commit('Bug 2058227 - Integrate the thing', [2058227]))).toBe(
      'Integrate the thing',
    );
  });
  ...
  it('accepts a full stop separator', () => {
    expect(commitTitle(commit('Bug 2055222. Add qcms_profile_create', [2055222]))).toBe(
      'Add qcms_profile_create',
    );
  });

  it('accepts a run-on message when the next word starts a sentence', () => {
    expect(commitTitle(commit('Bug 2057712 Update android nightly', [2057712]))).toBe(
      'Update android nightly',
    );
  });
  ...
  it('keeps a run-on message that continues into another bug', () => {
    expect(commitTitle(commit('Bug 12345 and bug 6789 - fix both', [12345, 6789]))).toBe(
      'Bug 12345 and bug 6789 - fix both',
    );
  });
  ...
  it('keeps the text when the leading bug is not the linked one', () => {
    expect(commitTitle(commit('Bug 111111 - part two', [222222]))).toBe('Bug 111111 - part two');
  });

  it('keeps a summary that does not start with a bug', () => {
    expect(commitTitle(commit('Backed out changeset abc', []))).toBe('Backed out changeset abc');
  });

  it('keeps a summary whose bug is mentioned mid-sentence', () => {
    expect(commitTitle(commit('Follow-up to bug 2058227 - tweak', [2058227]))).toBe(
      'Follow-up to bug 2058227 - tweak',
    );
  });
});
...
describe('pushlogLabel', () => {
  ...
  it('shows shown-of-total when a merge hid some', () => {
    expect(pushlogLabel(range({ commits: commits(20), hiddenRevisions: 144 }))).toBe(
      '20 of 164 commits',
    );
  });
  ...
  it('marks the total as a floor when the range was truncated', () => {
    expect(pushlogLabel(range({ commits: commits(20), hiddenRevisions: 144, truncated: true }))).toBe(
      '20 of 164+ commits',
    );
  });

  it('marks a floor even when nothing was hidden within a push', () => {
    expect(pushlogLabel(range({ commits: commits(200), truncated: true }))).toBe(
      '200 of 200+ commits',
    );
  });
});
...
describe('pushlogCaveat', () => {
  ...
  it('explains a capped merge', () => {
    expect(pushlogCaveat({ ...base, hiddenRevisions: 144 })).toMatch(/144 further commits/);
  });

  it('singularizes one hidden commit', () => {
    expect(pushlogCaveat({ ...base, hiddenRevisions: 1 })).toMatch(/1 further commit is/);
  });

  // Truncation wins: when both are true the range cap is the bigger lie.
  it('reports truncation ahead of hidden revisions', () => {
    expect(pushlogCaveat({ ...base, hiddenRevisions: 5, truncated: true })).toMatch(
      /newest 200 pushes/,
    );
  });
});
